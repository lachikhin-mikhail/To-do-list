...
func isNextMonth(now time.Time, startDate time.Time, dates []int) bool {
...
func isNextYear(isNextMonth bool, now time.Time, startDate time.Time, months []int) bool { // Заменить на поиск через slices.Index
	if !isNextMonth {
		return false
	}
